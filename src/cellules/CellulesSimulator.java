...
public class CellulesSimulator implements Simulable {
     ...
     */ 
    public CellulesSimulator(int n, int m, int t, GUISimulator g){
     ...
     */
    private void setTaileCellule(int t){
    ...
    public Color getColorEtat(int e){
        ...
        if( e == 6 ) return Color.GREY;
        if( e == 7 ) return Color.ORANGE;
        if( e == 8 ) return Color.MAGENTA
        if( e == 9 ) return Color.DARK_GREY;
    ...
    public void affiche(){
        ...
        for(int i=0; i<(this.getPlateau().getNbL()); i++){
            for(int k=0; k<(getPlateau().getNbL()); k++){
                c = this.getColorEtat(this.getCellule(i, k));
